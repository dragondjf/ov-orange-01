...
document.getElementById("update_image").value=conf.mapUrl;//导入map图片路径
...
function getMapImg(){
    ...
    var imgurl=config['mapUrl'];//读取配置文件
    if(!imgurl){
...
function Global_SaveUserConfig()//保存用户配置信息
{
	...
	var conf={};
	conf.dc = [];
        thiz.dc_key = ["did","product_type","ipaddr","name","cx","cy","medium_type","sensitivity","sensitivity2","env_factor","sid","mac","desc","status"];
	thiz.dc_key.push("pa_num","latest_change_time","hw_version","sw_version","protocol_ver");
    thiz.pa_key = ["did","pid","name","desc","enable","cx","cy","work_mode",
	...
	thiz.pa_key.push("sid","status","process_mode");
	if(!GREUtils.File.exists(userfilepath))//用户配置文件不存在
	{	  
          //新建custom.conf
	  GREUtils.Dir.getFile(userfilepath);//创建文件
	}
	    for (var i=0; i< Ifpms.Xpcom.DCMgmt.dc_list.length; i++) {
            ...
            conf.dc.push(dc_conf);
	}	    
	    conf.userinfo=get_alluserinfo();//保存用户账号信息
        conf.mapUrl=getMapImgURL();//地图背景图片
        conf.paIcon=getPaIconArray();//防区图标大中小
       // jsdump("bbbbbbbbbbbbbbbbbbbbbbbb"+wavechecked);
        conf.waveChecked=wavechecked;
        conf.alarmSoundtime=alarmSoundtime;
		conf.alarmMax=global_alarmlistlength;//最大告警长度
		conf.wavestart=document.getElementById('sampleNumber').value;//波形起点
		conf.waveend=document.getElementById('sample_end').value;//波形终点
		conf.wavelayout=document.getElementById('layoutType').value;
		conf.alarmprocmode=global_alarmproc_mode;
		var canids=['wave_canvas','sec_wave','third_wave','fourth_wave'];//波形参数设置信息
		conf.wavesetting=[];
		for(var i in canids)
		{
		  conf.wavesetting.push(document.getElementById(canids[i]).saveConfig());
		}
	    GREUtils.JSON.encodeToFile(userfilepath,conf);	
}
