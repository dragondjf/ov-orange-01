...
DC_IPADDR = "192.168.100.100"	#采集器的IP地址
								# v1.0采集器 IP一般为192.168.100.100, 主机必须设置为192.168.100.120
								# v0.5采集器 IP一般为192.168.10.178, 主机必须设置为192.168.10.12
DC_TYPE = 2						#采集器类型 0-v0.5硬件，1-v1.0硬件, 2-与管理软件同时运行，监听网络报文
PA_ID = 1						#防区编号，1或2
DSIZE = 512						#数据长度，x轴大小，一般不需改动
...
if DC_TYPE == 0 or DC_TYPE == 1:
	dc =nsIPyIfpmsDC()
	dc.init(nsIPyIfpmsDC.Type_Normal, DC_IPADDR, 2, 1)
else:
	dc =nsIPyIfpmsDC()
	dc.init(nsIPyIfpmsDC.Type_NetCaptor, DC_IPADDR, 2, 1)

TSIZE = 10
TCOLOR=['b','r','g','y','h','b','g','y','r','h']
TNAME = ['min', 'max', 'freq', 'spread', 'prewarn'] + ['data%d'%n for n in range(0,TSIZE)]
LINE_NAMES = [['min','max'],['data%d'%n for n in range(2,4)],['data5'],['data%d'%n for n in range(6,8)],['freq'],['prewarn']]
data = {}
...
try:
    if DC_TYPE == 0:
        serv = Serv.DcTcpServer(('',6001), Serv.DcTcpHandler, None, None)
    elif DC_TYPE == 1:
        serv = Serv.DcTcpServer(('',6002), Serv.NewDcTcpHandler, None, None)
    else: 
        serv = Serv.DcTcpServer(('',6000), Serv.NewDcTcpHandler, None, None)

    serv.add_dc(DC_IPADDR,1)
...
def update_line(event):
    ...
    data["offset"], data['min'], data['max'], data['freq'], data['spread'], data['prewarn'], data['data0'], data['data1'], data['data2'] ,\
        data['data3'], data['data4'], data['data5'], data['data6'], data['data7'], data['data8'], data['data9'] = dc.collector.getSamplingAll(PA_ID,-1*DSIZE)
