...
class Collector:
    ...
    def sampling(self, pid, dmin, dmax, davg, variance, compression, dfreqs):
        ...
        if (self.pa[pid - 1].work_mode == 4 or self.pa[pid - 1].work_mode == 2) and self.dc.Q_workmode == 0:
        ...
        elif (self.pa[pid - 1].work_mode == 4 or self.pa[pid - 1].work_mode == 2) and self.dc.Q_workmode == 1:
            #取最近2分钟max的最大值，并且正常情况下max值不应低于500
    ...
    def check_wenbo(self, pid, status_orig):
        ...
        S_NUM = int(ifpms.wb_settings['SRange'][daynight] * pkt_num_pre_sec)
        L_NUM = int(ifpms.wb_settings['LRange'][daynight] * pkt_num_pre_sec)
        THRESHOLD_FREQ = self.pa[pid - 1].alarm_sensitivity + ifpms.wb_settings['LowFreqThreshold'][daynight]
        WarnRatio = self.pa[pid - 1].alarm_resistant_factor_gsd + ifpms.wb_settings['WarnRatio'][daynight]

        #取最近2分钟max的最大值，并且正常情况下max值不应低于500
        t_max = max(np.max(self.data[pid - 1, -5 * 60 * 2:]["max"]), 500)
        #屏蔽光功率绝对值对F的影响
        self.data[pid - 1, -1]['data0'] = t_max
        self.data[pid - 1, -1]['data1'] = self.data[pid - 1, -1]['freq']
        self.data[pid - 1, -1]['freq'] = self.data[pid - 1, -1]['data1'] * 100 / t_max

        if self.data[pid - 1, -1]['freq'] >= ifpms.wb_settings['HighFreqThreshold'][daynight]:
            ...
            if self.offset[pid - 1] % S_NUM == 0:  # 达到检查点，按时间分段，计算每小段的告警状态，小段告警为轻微告警
                ...
                n0 = np.sum(np.greater(self.data[pid - 1, -1 * S_NUM:]['freq'], THRESHOLD_FREQ))
                self.data[pid - 1, -1]['data2'] = n0
                #n1 = np.mean(self.data[pid - 1, -1 * S_NUM:]['freq'])
                #self.data[pid - 1, -1]['data3'] = n1
                #self.data[pid - 1, -1]['data4'] = n1 * 100 / np.max(self.data[pid - 1, -1 * S_NUM:]['freq'])
                #平滑度
                #n1 = np.sum(np.greater(self.data[pid - 1, -1 * S_NUM:]['data5'], ifpms.wb_settings['HighThreshold'][daynight]))
                #n2 = np.sum(np.greater(self.data[pid - 1, -1 * S_NUM:]['data5'], ifpms.wb_settings['LowThreshold'][daynight]))

                #logger.info(self.data[pid-1,-1*S_NUM:]['data5'])
                if n0 > ifpms.wb_settings['SRange'][daynight] * 2:  # 每秒超过门限两次以上，认为是不间断干扰源
                    if ifpms.rainstorm_suppression:
                        #抑制一半的报警
                        self.data[pid - 1, -1]['data6'] = 5 * (self.offset[pid - 1] / S_NUM % 2 * 2 - 1)
                    else:
                        #轻微报警
                        self.data[pid - 1, -1]['data6'] = -3
                        status = Collector.STATUS_ALARM_MINOR
                elif n0 >= ifpms.wb_settings['LowFreqThresholdTime'][daynight]:
                    self.data[pid - 1, -1]['data6'] = 5
                    #status = Collector.STATUS_ALARM_MINOR  #不再向前端输出预警
                else:
                    self.data[pid - 1, -1]['data6'] = -5
                    status = Collector.STATUS_CONNECT

                #统计长时间段内的告警次数，超过门限为严重告警
                n3 = np.sum(np.greater(self.data[pid - 1, -1 * L_NUM:]['data6'], 0))
                self.data[pid - 1, -1]['data7'] = n3 * 100 / (L_NUM / S_NUM)

                if self.data[pid - 1, -1]['data7'] >= WarnRatio:
                    logger.info("critical alarm: %d > %d", self.data[pid - 1, -1]['data7'], WarnRatio)
                    status = Collector.STATUS_ALARM_CRITICAL
