...
for skin_item in gsd default gsd-ov
do
	...
	if [ ${skin_item} == 'gsd-ov' ]; then
        echo "logo = 'gsd'" >> extensions/ifpms@ov-orange.com/pylib/mark.py
	else
    ...
    if [ ${skin_item} == 'default' ]; then
		...
		cp ../setup.nsi .
		sed 's/PRODUCT_NAME "ifpms"/PRODUCT_NAME "gsd"/g' setup.nsi > gsdsetup.nsi
		makensis gsdsetup.nsi
        mv setup-v1.2-r${BUILD}-b${DATE}.exe ../${skin_item}-v1.2-r${BUILD}-b${DATE}.exe
		rm setup.nsi gsdsetup.nsi
    fi
done
cd ..
echo "Job Done!"
