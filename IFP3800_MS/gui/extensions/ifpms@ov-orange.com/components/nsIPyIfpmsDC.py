...
class nsIPyIfpmsDCMgmt:
    ...
    def removeDC(self, did):
        ...
        while True:
            alarm_idx = -1
            for i in xrange(0, self.alarm_history.length):
                t = self.alarm_history.queryElementAt(i, components.interfaces.nsIPyIfpmsAlarmRecord)
                if t and t.did == did:
                    alarm_idx = i
                    break
            if alarm_idx != -1:
                self.alarm_history.removeElementAt(alarm_idx)
            else:
                break        
