...
Blasting_P = {
    '192.168.100.4': [[4000, 5000], [4000, 5000]],
    '192.168.100.5': [[4000, 5000], [4000, 5000]]
}
#模式设置
fft_window_settings = {
    # 1 标准周界
...
magic = 1  # 标准周界算法：freq值除以该值使其变化范围在0-300之间
...
# Web Service 配置
webs_idle_seconds = 30  # 空闲时间,单位秒
resend_times = 3  # 发送失败后重新发送的次数
resend_interval = 10  # 重新发送时间（单位秒）
...
#标准周界模式
action_n = 1    # 特征值积分点数
background_t1 = 5  # 背景特征起始时间，单位为s, 每秒计算5个点
background_t2 = 10  # 背景特征结束时间，单位为s, 每秒计算5个点
b = 3   # 背景与动作的区别系数
k = 20  # 归一化系数
...
# 自适应周界特征值选择标识，仅仅采集器模式有效
featureflag = True  # 为True表示采用频域特征，为False表示采用的是时域特征
