...
Ifpms.PaMgr = {
  ...
  pa_delete: function pa_delete() {
    ...
    if ( GREUtils.Dialog.confirm(bundle.getString("delComfirm"),bundle.getString("delContent")) ){
      ...
      alarmmgmt.tablemanager.redrawTable();
      alarmmgmt.tablemanager.showNewestAlarm();

      //未处理的吿警中删除关联告警
      var l = global_unpro_alarms.length;
      for(var i = 0; i < l; i++) {
        if(global_unpro_alarms[i].did==did) {
          GREUtils.log("remove alarm "+global_unpro_alarms[i].alarmid);
